...
if args.semantics:
    with open(args.pallete, "r") as f:
        pallete = json.load(f)
    semantic_classes = pallete["semantic_classes"]
    label_mapping_nyu = load_scannet_nyu13_mapping(basedir)
    os.makedirs(os.path.join(basedir, 'label_13'), exist_ok=True)
    os.makedirs(os.path.join(basedir, 'label_13_scaled'), exist_ok=True)


train_ids = frame_ids[::step]
test_id_step = 10
test_ids = [x+ (test_id_step//2) for x in train_ids]
    ...
    for frame_id in ids:
        ...
        if args.scaled_image:
            ...
            if args.semantics:
                ...
                #     semantic[semantic_copy == semantic_classes[i]] = i
                semantic_scaled = cv2.copyMakeBorder(src=semantic, top=2, bottom=2, left=0, right=0, borderType=cv2.BORDER_CONSTANT, value=0)
                semantic_scaled = cv2.resize(semantic_scaled, (W, H), interpolation=cv2.INTER_NEAREST)
                semantic = semantic.astype(np.uint8)
                semantic_scaled = semantic_scaled.astype(np.uint8)
                cv2.imwrite(os.path.join(basedir, 'label_13_scaled', '%d.png'% frame_id), semantic_scaled)
                cv2.imwrite(os.path.join(basedir, 'label_13', '%d.png'% frame_id), semantic)


        json_image_dict ={}
        json_image_dict["file_path"] = os.path.join('color_scaled', '%d.jpg'% frame_id)
        if args.semantics:
            json_image_dict["label_path"] = os.path.join('label_13_scaled', '%d.png'% frame_id)
        ...
        if args.semantics:
            json_image_dict_unscaled["label_path"] = os.path.join('label_13', '%d.png'% frame_id)
